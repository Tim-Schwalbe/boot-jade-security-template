...
//      .and().authorizeRequests().antMatchers("/profile/password**").permitAll()
...
//      .and().formLogin().loginPage("/login").defaultSuccessUrl("/landingpage", true).failureUrl("/login?error=true").usernameParameter("username").passwordParameter("password")
...
//      .and().exceptionHandling().accessDeniedPage("/err/403");
        ...
        http.exceptionHandling().accessDeniedPage("/err/403").and().authorizeRequests().antMatchers("/login").anonymous().and()
                .authorizeRequests().antMatchers("/imprint").permitAll().and()
                .authorizeRequests().antMatchers("/profile/password**").permitAll()
                .and().authorizeRequests().antMatchers("/forgot_password").anonymous().and().authorizeRequests().antMatchers(
                "/triggeredBy/password**").permitAll().and().authorizeRequests().antMatchers("/err/403").permitAll().and().authorizeRequests()
                .antMatchers("/**").hasRole("PROFILE").and().formLogin().loginPage("/login").defaultSuccessUrl("/", true).failureUrl(
                "/login?error=true").usernameParameter("username").passwordParameter("password").and().logout().logoutUrl("/logout")
                .logoutSuccessUrl("/login?logout").and().rememberMe().rememberMeCookieName("REMEMBER_ME").rememberMeParameter("remember_me")
                .tokenValiditySeconds(123456).key("49874795145977617241");
